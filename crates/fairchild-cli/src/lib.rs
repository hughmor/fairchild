...
//! thirteen places, which is correct in a binary and fatal in an extension
    ...
    #[cfg(feature = "osdi")]
    {
        ...
        for osdi_path in &netlist.osdi_paths {
            ...
            let lib = unsafe { OsdiLibrary::open(&path) }
                .map_err(|e| format!("cannot load OSDI library '{}': {e}", path.display()))?;
            let lib = Arc::new(lib);
...
/// exit code. Never exits the process, never panics on user error — see the
...
/// usage-error code; a failed simulation returns 1.
    ...
    T: Into<OsString> + Clone,
{
    let cli = match Cli::try_parse_from(args) {
        Ok(cli) => cli,
        Err(e) => {
            // clap already picks the stream: help/version to stdout, the rest
            // to stderr.
            let _ = e.print();
            return match e.kind() {
...
fn run_cli(cli: Cli) -> Result<i32, String> {
    let mut netlist = parse_spice_file(&cli.file).map_err(|e| e.to_string())?;
...
fn open_writer(path: &Path) -> Result<Box<dyn Write + Send>, String> {
...
fn run_corners_parallel(
    ...
    // `collect::<Result<_, _>>` over a parallel iterator keeps the first error
    // and cancels the rest; without it a corner that fails on one thread would
    // have to kill the process to be noticed at all.
    ...
    for analysis in &netlist.analyses {
        match analysis {
            ...
            Analysis::Tran {
                ...
                .map_err(|e| format!("tran failed: {e}"))?;
                if ctx.verbose {
                    eprintln!(
