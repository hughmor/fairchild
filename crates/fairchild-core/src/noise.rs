...
impl NoiseResult {
    ...
    pub fn write_csv<W: std::io::Write>(&self, mut w: W) -> std::io::Result<()> {
        writeln!(w, "freq_hz,onoise_v2hz,onoise_vrthz,inoise_v2hz,inoise_vrthz")?;
        for i in 0..self.freq.len() {
            let on = self.onoise_psd[i];
            let in_ = self.inoise_psd[i];
            writeln!(w, "{:.6e},{:.6e},{:.6e},{:.6e},{:.6e}",
                self.freq[i], on, on.max(0.0).sqrt(), in_, in_.max(0.0).sqrt())?;
        }
        Ok(())
    }
}
...
pub fn noise_analysis(
    ...
    for el in &netlist.elements {
        match el {
            Element::Capacitor { pos, neg, capacitance, .. } =>
                stamp_passive_2port(&mut c_mat, &topo.node_index, pos, neg, *capacitance),
            Element::Inductor { pos, neg, inductance, .. } =>
    ...
    let input_vsrc_idx = topo.vsrc_index.get(input_src).copied()
        .ok_or_else(|| SimError::ParameterError(
            format!("noise: input source '{input_src}' not found")
        ))?;

    let out_pos_idx = if out_pos == "0" { None } else { topo.node_index.get(out_pos).copied() };
    let out_neg_idx = if out_neg == "0" { None } else { topo.node_index.get(out_neg).copied() };
    ...
    for &f in freqs {
        ...
        let s_v_in = if h_mag_sq > 1e-30 { s_v_out / h_mag_sq } else { f64::NAN };
