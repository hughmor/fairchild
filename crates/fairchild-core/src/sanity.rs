...
pub fn check_netlist_sanity(netlist: &Netlist) -> usize {
    ...
    for w in &warnings {
        if !by_cat.contains_key(w.category) { order.push(w.category); }
    ...
    for cat in order {
        ...
        if group.len() <= SHOW_FULL {
            ...
            let extras: Vec<&str> = group[SHOW_FULL..]
                .iter().map(|w| w.refdes.as_str()).collect();
            let truncated = if extras.len() > 10 {
                format!("{}, … (+{} more)",
                    extras[..10].join(", "), extras.len() - 10)
            } else {
                extras.join(", ")
            };
            eprintln!("warning: sanity-check: …and {} more with the same issue \
                       ('{cat}'): {truncated}",
                extras.len());
        }
    }
        ...
        match el {
            Element::Resistor { name, resistance, .. } => {
                if !resistance.is_finite() {
                    out.push(Warning { category: "R-nonfinite", refdes: name.clone(),
                        detail: format!("{name}: R={resistance} is non-finite — \
                                         stamp will produce NaN/Inf") });
                } else if *resistance == 0.0 {
                    out.push(Warning { category: "R=0", refdes: name.clone(),
                        detail: format!("{name}: R=0 → stamp will produce 1/0 = ∞ \
                                         (use a small positive value like 1m, or \
                                         replace with a wire by merging the nets)") });
                } else if *resistance < 0.0 {
                    out.push(Warning { category: "R-negative", refdes: name.clone(),
                        detail: format!("{name}: R={resistance} is negative \
            ...
            Element::Capacitor { name, capacitance, .. } => {
                if !capacitance.is_finite() {
                    out.push(Warning { category: "C-nonfinite", refdes: name.clone(),
                        detail: format!("{name}: C={capacitance} is non-finite") });
                } else if *capacitance < 0.0 {
                    out.push(Warning { category: "C-negative", refdes: name.clone(),
            ...
            Element::Inductor { name, inductance, .. } => {
                if !inductance.is_finite() {
                    out.push(Warning { category: "L-nonfinite", refdes: name.clone(),
                        detail: format!("{name}: L={inductance} is non-finite") });
                } else if *inductance == 0.0 {
                    out.push(Warning { category: "L=0", refdes: name.clone(),
                        detail: format!("{name}: L=0 → transient companion uses \
                                         h/L which is undefined (DC OP treats it \
                                         as open — fine for `.op`, but transient \
                                         will fail)") });
                } else if *inductance < 0.0 {
                    out.push(Warning { category: "L-negative", refdes: name.clone(),
...
fn check_source_amplitudes(netlist: &Netlist, out: &mut Vec<Warning>) {
    for el in &netlist.elements {
        ...
        if let Some(bad) = waveform_non_finite(wf) {
            out.push(Warning { category: "source-nonfinite", refdes: name.clone(),
...
fn waveform_non_finite(wf: &Waveform) -> Option<String> {
    let chk = |v: f64, what: &str| -> Option<String> {
        if !v.is_finite() { Some(format!("{what}={v}")) } else { None }
    };
    match wf {
        ...
        Waveform::Pwl { points } => {
            for (t, v) in points {
                if !t.is_finite() { return Some(format!("PWL t={t}")); }
                if !v.is_finite() { return Some(format!("PWL v={v}")); }
            }
            None
        }
...
fn check_self_shorts(netlist: &Netlist, out: &mut Vec<Warning>) {
    for el in &netlist.elements {
        let (name, pos, neg, kind) = match el {
            ...
            Element::VoltageSource { name, pos, neg, .. } => (name, pos.as_str(), neg.as_str(), "V"),
            Element::CurrentSource { name, pos, neg, .. } => (name, pos.as_str(), neg.as_str(), "I"),
            _ => continue,
        };
        if pos.eq_ignore_ascii_case(neg) {
            ...
            out.push(Warning { category: "self-short", refdes: name.clone(),
                detail: format!("{name} ({kind}): both terminals tied to net \
...
fn check_duplicate_refdes(netlist: &Netlist, out: &mut Vec<Warning>) {
    ...
    let mut dups: Vec<(&String, &usize)> = seen.iter()
        .filter(|(_, &n)| n > 1)
        .collect();
    dups.sort_by(|a, b| a.0.cmp(b.0));
    for (name, count) in dups {
        out.push(Warning { category: "duplicate-refdes", refdes: name.clone(),
            detail: format!("refdes '{name}' appears {count} times — \
...
fn check_fc_xosdi_params(netlist: &Netlist, out: &mut Vec<Warning>) {
    for el in &netlist.elements {
        let (name, model, params) = match el {
            Element::XOsdi { name, model_name, params, .. } => (name, model_name, params),
            _ => continue,
        };
        let model_lc = model.to_lowercase();
        let bad_keys = fatal_zero_params(&model_lc);
        if bad_keys.is_empty() { continue; }
        for (key, value) in params {
            let key_lc = key.to_lowercase();
            if bad_keys.iter().any(|&k| k == key_lc) {
                if *value == 0.0 {
                    out.push(Warning { category: "fc-zero-param", refdes: name.clone(),
                        detail: format!("{name} ({model}): {key}={value} → \
                                         stamp will produce 1/0 = ∞ (this param \
                                         appears in a reciprocal in the device's \
                                         eval/jacobian)") });
                } else if !value.is_finite() {
                    out.push(Warning { category: "fc-nonfinite-param", refdes: name.clone(),
                        detail: format!("{name} ({model}): {key}={value} is non-finite") });
                } else if *value < 0.0 {
                    out.push(Warning { category: "fc-negative-param", refdes: name.clone(),
                        detail: format!("{name} ({model}): {key}={value} is negative \
