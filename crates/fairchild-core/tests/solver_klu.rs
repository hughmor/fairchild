...
fn klu_solves_resistor_divider() {
    ...
    assert!((v_out - 0.5).abs() < 1e-9, "V(out) = {v_out} (expected 0.5)");
}
...
fn klu_options_directive_selects_klu_backend() {
    ...
    let r = dc_op_nr_with_registry_opts(&net, &reg, &opts).unwrap();
    let v_out = r.node_voltage("out").unwrap();
    assert!((v_out - 1.0).abs() < 1e-9, "V(out) = {v_out} (expected 1.0)");
}
...
fn klu_transient_diode_rc_matches_dense() {
    ...
    let r_klu = tran_nr_with_registry_var_opts(
        &net, 0.5e-9, 30e-9, &reg, &opts_klu,
    ).unwrap();

    let mut opts_dense = SimOptions::from_netlist(&net);
    opts_dense.solver = SolverKind::Dense;
    let r_dense = tran_nr_with_registry_var_opts(
        &net, 0.5e-9, 30e-9, &reg, &opts_dense,
    ).unwrap();

    let v_klu_last = r_klu.node_voltages.get("out").unwrap().last().copied().unwrap();
    let v_dense_last = r_dense.node_voltages.get("out").unwrap().last().copied().unwrap();
    assert!((v_klu_last - v_dense_last).abs() < 1e-6,
