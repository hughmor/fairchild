...
fn the_unmodelled_report_names_the_parameter_and_the_consequence() {
    let card = vec![
        ("is".to_string(), 1e-16),
        ("ikf".to_string(), 1e-3), // modelled now — must NOT be reported
        ("kf".to_string(), 1e-15), // not modelled
        ("cjs".to_string(), 1e-12),
    ];
    let lines = unmodelled::report(unmodelled::BJT, &card);
    assert_eq!(lines.len(), 2, "expected KF and CJS only, got {lines:?}");
    assert!(lines.iter().any(|l| l.starts_with("KF ignored:")));
