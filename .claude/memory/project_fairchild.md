...
| **Perf-2 — Symbolic/numeric factorisation split** | ✅ done 2026-05-21 | `LinearSolver` trait gains a `factorise(&matrix)` method that returns a reusable `Factorisation` handle. Newton-Raphson (DC OP + transient fixed-step + transient variable-step) build one factorisation per solve and call `refactor_and_solve` per iteration — the KLU backend then runs `klu_refactor` on every NR step instead of full `klu_factor`. Pattern-growth detection: if the dense matrix grows a new non-zero entry between iterations (photonic devices at x=0 emit zeros that activate at the operating point), the cache re-analyses transparently. FaerSparseSolver path delegates to one-shot solve until faer exposes a refactor primitive. Trait default impl is a `NoCacheFactorisation` so backends opt in by overriding `factorise`. |
| **Perf-3 — Reused MnaMatrix across NR / transient iterations** | ✅ done 2026-05-21 | New `MnaMatrix::clear()` + in-place stamping helpers (`stamp_netlist_in_place`, `stamp_netlist_scaled_in_place`); the DC OP NR loop, fixed-step transient NR loop, and variable-step transient NR loop all hoist matrix allocation outside the loop and reuse a single matrix. Saves N+1 heap allocations per NR iteration — for a 10 k-step transient with ~3 NR iters that's ~30 k × (N+1) allocations skipped. The allocating `stamp_netlist` / `stamp_netlist_scaled` shapes are kept for non-hot call sites (DC OP probes, AC G-matrix assembly, noise side-car). |
| **Perf-4 — Parallel `.alter` × `.temp` corner runs** | ✅ done 2026-05-21 | The CLI's nested alter/temp loop was hoisted into a flat `Vec<Corner>` and dispatched two ways. **File-per-corner (new default)** when `--output PATH` is given, more than one corner exists, and neither `--single-output` nor `--verbose` is set — corners run in parallel via rayon, each writing to `PATH.alter_LABEL.temp_Cc.ext`. **Single-output (opt-in via `--single-output`)** preserves the historic concatenated layout with `# alter=…` / `# temp_c=…` separator headers, serial only so ordering is deterministic. Stdout output and verbose-mode runs also fall back to single-output. Three-surface: `.alter` / `.temp` parser unchanged; CLI gains `--single-output`; Python path unaffected (Python users iterate corners explicitly at the Python level by re-parsing). |
| **Fix-1 — Variable-step transient breakpoint restart** | ✅ done 2026-05-21 | `tran_nr_with_registry_var_opts` was infinite-looping on the LTE rejection path right after a waveform slope discontinuity (e.g. the rising-edge→plateau kink of a PWL/PULSE source). The linear predictor used `x_prev` from the steep-slope side of the kink and overestimated `x_pred`; the LTE estimator then attributed the predictor overshoot to "integration error" and demanded h → 0 (1500× shrinkage without LTE budging). Standard SPICE practice — restart the integrator across a known breakpoint — was missing. Fix: track `just_crossed_breakpoint` when an accepted step lands on a tracked waveform breakpoint, and on the next step use zeroth-order prediction (`x_pred = x`) and skip LTE evaluation (auto-accept on NR convergence). All three integrators (BE / TR / Gear) now complete the WDM transient regression test. |
| **CI (Option A)** | ✅ done 2026-05-27 | `.github/workflows/ci.yml`: matrix ubuntu/macos, installs SuiteSparse, runs fmt/clippy(-D warnings)/test (default + `--features klu`) and bench compile-check. Commit `837eb09`. |
| **BJT (Option B)** | ✅ done 2026-05-27 | Gummel-Poon Level 1 NPN/PNP: parser `Q<n> C B E [S] model`, `.model NPN/PNP` with IS/BF/BR/NF/NR/VAF/VAR/TF/TR; core model with polarity trick + Early voltage + pnjlim + transit-time transient charges; DeviceRegistry + newton/mna/connectivity/sanity wired; Python `register_builtin_bjts` called. 3 unit tests. Commit `db10059`. |
| **K element — coupled inductors** | ✅ done 2026-05-27 | `K<n> L1 L2 coupling` mutual-inductance element. Parser: `CoupledInductors` variant in `Element` enum, `'k'` arm in `parse_element`, coupling validated in [0,1]. MNA: 2×2 BE companion (G11/G22/G12 from det = L1·L2 − M²), per-inductor diagonal replaced by coupled stamp when K element present; `stamp_mutual_conductance` helper. Tran: `advance_companions` snapshots i_hist before main loop, K post-pass corrects il1/il2 using 2×2 formula; post-pass gated on `BackwardEuler | Gear` to avoid corrupting TR state. Wired into connectivity/sanity/newton match arms. 3 tests in `tests/coupled_inductors.rs`: k=0 decoupled, 1:1 transformer (V(n2)/V(n1) ratio within 10%), LC resonance frequency shift (zero-crossing ratio ≈ √1.5). All 270+ workspace tests pass. |
