...
Output selection is `--probe` (CLI — CSV for every analysis, and a `.tran`
rawfile; a rawfile from a bounded analysis carries every signal and warns that
it has) or indexing the returned result (Python), so
